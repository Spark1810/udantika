...
def get_difference(df: pd.DataFrame, num_features: List[str]) -> pd.DataFrame:
    ...
    df_diff = (
        df.loc[:, num_features + ["patient_ID"]]
        .groupby(["patient_ID"])
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    all_cols = [c for c in list(df.columns) if c not in ["patient_ID", "S_2"]]
    ...
    df_num_agg = df.groupby("patient_ID")[num_features].agg(["first", "mean", "std", "min", "max", "last"])
    df_num_agg.columns = ["_".join(x) for x in df_num_agg.columns]
    df_num_agg.reset_index(inplace=True)

    df_cat_agg = df.groupby("patient_ID")[cat_features].agg(["count", "first", "last", "nunique"])
    ...
    df = df_num_agg.merge(df_cat_agg, how="inner", on="patient_ID").merge(df_diff_agg, how="inner", on="patient_ID")
...
def _main(cfg: DictConfig) -> NoReturn:
    ...
    train = pd.read_parquet(path / "input/prediction/train_meta.parquet")
    label = pd.read_csv(path / "input/prediction/train_labels.csv")
    test = pd.read_parquet(path / "input/prediction/test_meta.parquet")
    path = Path(get_original_cwd()) / cfg.dataset.path

    # build train features
    train = build_features(train)
    train = pd.merge(train, label, how="left", on="patient_ID")
    ...
    # build test features
    split_ids = split_dataset(test.patient_ID.unique(), cfg.dataset.num)

    for (i, ids) in enumerate(split_ids):
        test_sample = test[test.patient_ID.isin(ids)]
